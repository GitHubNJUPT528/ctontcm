...
public abstract class GeneticAlgorithm extends Chromosome {
    private List<Chromosome> population = new ArrayList<>();
    private int popSize = 200;//种群数量
    ...
    public void calculate(double cw[], double w[], double v[]){
        generation = 1;
        init(cw,w,v);
        while (generation < maxIterNum) {  //迭代maxIterNum
            evolve(cw,w,v);   // （选择 -> 交叉)+ -> 变异 -> 计算得分
            print();    // 打印
            generation++;  // 代数
        }
    }

    private void evolve(double cw[], double w[], double v[]) {
        ...
        while (childPopulation.size() < popSize) {
            ...
            Chromosome cchromosome = new Chromosome();
            List<Chromosome> chromosomes =cchromosome.genetic(p1, p2, crossRate);
            if (chromosomes != null)
    ...
    private void init(double cw[], double w[], double v[]){
        population = new ArrayList<>();
        for (int i = 0; i < popSize; i++) {
            Chromosome chromosome = new Chromosome(geneSize,cw,w);
            chromosome.setCw(cw);
            chromosome.setW(w);
            chromosome.setV(v);
            System.out.println("初始化"+i+chromosome);
    ...
    private void print() {
        ...
        System.out.println("种群"+population);
    }
